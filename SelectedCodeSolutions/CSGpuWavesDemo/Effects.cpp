...
WaveRenderEffect::WaveRenderEffect(ID3D11Device* device, const std::wstring& filename)
	: Effect(device, filename)
{
	Light3Tech               = mFX->GetTechniqueByName("Light3");
	Light3TexTech            = mFX->GetTechniqueByName("Light3Tex");
	...
	FogStart                 = mFX->GetVariableByName("gFogStart")->AsScalar();
	FogRange                 = mFX->GetVariableByName("gFogRange")->AsScalar();
	GridSpatialStep          = mFX->GetVariableByName("gGridSpatialStep")->AsScalar();
	DisplacementMapTexelSize = mFX->GetVariableByName("gDisplacementMapTexelSize")->AsVector();
	DirLights                = mFX->GetVariableByName("gDirLights");
	Mat                      = mFX->GetVariableByName("gMaterial");
	DiffuseMap               = mFX->GetVariableByName("gDiffuseMap")->AsShaderResource();
...
WaveRenderEffect::~WaveRenderEffect()
{
}
#pragma endregion
...
WaveSimEffect::WaveSimEffect(ID3D11Device* device, const std::wstring& filename)
	: Effect(device, filename)
{
	UpdateWavesTech  = mFX->GetTechniqueByName("UpdateWaves");
	DisturbWavesTech = mFX->GetTechniqueByName("DisturbWaves");

	WaveConstants      = mFX->GetVariableByName("gWaveConstants")->AsScalar();
	DisturbMag         = mFX->GetVariableByName("gDisturbMag")->AsScalar();
	...
	CurrSolInput  = mFX->GetVariableByName("gCurrSolInput")->AsShaderResource();
	CurrSolOutput = mFX->GetVariableByName("gCurrSolOutput")->AsUnorderedAccessView();
	NextSolOutput = mFX->GetVariableByName("gNextSolOutput")->AsUnorderedAccessView();
}

WaveSimEffect::~WaveSimEffect()
{
}
#pragma endregion
...
WaveSimEffect*    Effects::WaveSimFX    = 0;
WaveRenderEffect* Effects::WaveRenderFX = 0;

void Effects::InitAll(ID3D11Device* device)
{
	BasicFX = new BasicEffect(device, L"FX/Basic.fxo");
	BlurFX  = new BlurEffect(device, L"FX/Blur.fxo");
	WaveSimFX = new WaveSimEffect(device, L"FX/WaveSim.fxo");
	WaveRenderFX = new WaveRenderEffect(device, L"FX/WaveRender.fxo");
}
...
void Effects::DestroyAll()
{
	SafeDelete(BasicFX);
	SafeDelete(BlurFX);
	SafeDelete(WaveSimFX);
	SafeDelete(WaveRenderFX);
}
#pragma endregion
//***************************************************************************************
// TexColumnDemo.cpp by Frank Luna (C) 2011 All Rights Reserved.
//
// Demonstrates texturing the procedural geometry such as cylinders, grids, boxes, and spheres.
//
// Controls:
...
class TexColumnApp : public D3DApp 
{
public:
	TexColumnApp(HINSTANCE hInstance);
	~TexColumnApp();
...
TexColumnApp::TexColumnApp(HINSTANCE hInstance)
: D3DApp(hInstance), mShapesVB(0), mShapesIB(0), mSkullVB(0), mSkullIB(0), 
  mFloorTexSRV(0), mStoneTexSRV(0), mBrickTexSRV(0),
  mSkullIndexCount(0), mLightCount(1),
  mEyePosW(0.0f, 0.0f, 0.0f), mTheta(1.5f*MathHelper::Pi), mPhi(0.2f*MathHelper::Pi), mRadius(15.0f)
{
	mMainWndCaption = L"Tex-Column Demo";
...
void TexColumnApp::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void TexColumnApp::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void TexColumnApp::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void TexColumnApp::BuildSkullGeometryBuffers()
{

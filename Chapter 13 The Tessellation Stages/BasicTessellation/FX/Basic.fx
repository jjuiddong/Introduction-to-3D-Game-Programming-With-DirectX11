...
VertexOut VS(VertexIn vin, uniform bool gFogEnabled)
{
	VertexOut vout;
	...
	if( gFogEnabled )
	{
		// Do fog calculation at "cheaper" per vertex frequency.
		float distToEye = distance(vout.PosW, gEyePosW);
...
technique11 Light0Fog
{
    pass P0
    {
        SetVertexShader( CompileShader( vs_5_0, VS(true) ) );
        SetPixelShader( CompileShader( ps_5_0, PS(0, false, false, true) ) );
    }
}
